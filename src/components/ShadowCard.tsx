...
export const ShadowCard: React.FC<ShadowCardProps> = ({ data, onComplete }) => {
  // Görevlerin durumunu tutan State
  ...
  const handleSubmit = () => {
    setIsSubmitted(true);
    if (onComplete) {
      // Kullanıcının aldığı notları birleştirip ana sisteme gönderiyoruz
  ...
  return (
    <div className="w-full max-w-2xl bg-zinc-950 border-l-4 border-red-600 rounded-r-lg shadow-2xl p-6 my-6 animate-fade-in overflow-hidden relative">
      ...
      <div className="mb-8 relative z-10">
        ...
        <div className="space-y-4">
          {tasks.map((task, index) => (
            <div key={index} className={`p-4 rounded-lg border transition-all ${task.completed ? 'bg-green-900/10 border-green-900/30' : 'bg-zinc-900/30 border-zinc-800'}`}>
              {/* Checkbox ve Metin */}
              <div className="flex items-start gap-3 cursor-pointer" onClick={() => !isSubmitted && toggleTask(index)}>
                <div className={`mt-1 min-w-[20px] h-5 rounded border flex items-center justify-center transition-colors ${task.completed ? 'bg-green-600 border-green-600' : 'border-zinc-600'}`}>
                  {task.completed && <CheckCircle2 className="w-3.5 h-3.5 text-white" />}
                </div>
                <span className={`text-sm leading-relaxed ${task.completed ? 'text-zinc-400 line-through' : 'text-zinc-200'}`}>
                  {task.text}
                </span>
              </div>

              {/* Yorum Kutusu (Sadece seçiliyse veya not varsa açılır) */}
      ...
      {!isSubmitted ? (
        ...
        <div className="text-center p-3 bg-green-900/20 text-green-400 rounded border border-green-900/30 text-sm">
          Geri bildirimlerin sisteme işlendi. Dönüşüm süreci başlıyor...
        </div>
      )}
    </div>
  );
